...
formAgregarTipoingresos.addEventListener("submit", async function (event) {
    ...
    try {
        ...
        const response = await fetch('http://localhost:3009/ADB/create_tipo_egresos', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
        ...
        if (response.ok) {
            ...
            Toast.fire({
                icon: "success",
                title: "Se creo el Tipo de Egresos correctamente"
            });
            getAll()
        } else {
            // Actualizar la tabla después de cambiar el estado
...
const paginaTipoingreso = tipo_ingreso.querySelector('#tipo_egresos')

const TipoIngres = ({ id_tipo_egresos, nombre, registro_fecha, estado }) => {
    ...
    return `
        <tr id="Tipoingreso-row-${id_tipo_egresos}"> <!-- Agregar un ID único para la fila -->
...
const toggleEditMode = (id_tipo_egresos) => {
  const row = document.getElementById(`Tipoingreso-row-${id_tipo_egresos}`);
  ...
  if (!isEditMode) {
    // Modo de edición
    editTipingreso(id_tipo_egresos);
    editButton.innerHTML = 'Guardar';
    editButton.setAttribute('onclick', `toggleEditMode(${id_tipo_egresos}, ${JSON.stringify(valoresOriginales)})`);
    isEditMode = true;
  } else {
...
const editTipingreso = (id_tipo_egresos) => {
    const row = document.getElementById(`Tipoingreso-row-${id_tipo_egresos}`);
    const cells = row.getElementsByTagName('td');
    ...
    editButton.setAttribute('onclick', `toggleEditMode(${id_tipo_egresos}, ${JSON.stringify(valoresOriginales)}, this)`);
}
...
const saveChanges = async (id_tipo_egresos, valoresOriginales) => {
    ...
    try {
        ...
        if (isConfirmed) {
            ...
            const response = await fetch(`http://localhost:3009/ADB/tipo_egresos/${id_tipo_egresos}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`,
        ...
        Toast.fire({
            icon: "error",
            title: "Ocurrio un error al actualizar tipo de egreso"
        });
...
const changeState = async (userId, currentState) => {
    try {
        ...
        if (isConfirmed) {
            ...
            const response = await fetch(`http://localhost:3009/ADB/tipo_egresos/${userId}/state`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`,
            ...
            if (response.ok) {
                ...
                Toast.fire({
                    icon: "error",
                    title: "Error al cambiar el estado del tipo de egreso"
                }
                );
                getAll();
            }
...
const deleteUser = async (userId) => {
    try {
        ...
        if (isConfirmed) {
            ...
            const response = await fetch(`http://localhost:3009/ADB/tipo_egresos_delete/${userId}`, {
                method: 'DELETE',
                headers:{
                    Authorization: `Bearer ${token}`,
                }
            ...
            if (response.ok) {
                ...
                Toast.fire({
                    icon: "error",
                    title: "Error al eliminar el tipo de egreso"
                });
