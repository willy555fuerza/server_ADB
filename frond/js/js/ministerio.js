...
formAgregarUsuario.addEventListener("submit", async function (event) {
  ...
  try {
    ...
    const response = await fetch(
      "http://localhost:3009/ADB/create_ministerio",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
let tablaMinisterios; // Variable de control para DataTables
...
const saveChanges = async (id_ministerio, valoresOriginales) => {
  ...
  try {
    ...
    if (isConfirmed) {
      ...
      const response = await fetch(
        `http://localhost:3009/ADB/ministerio/${id_ministerio}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
...
const changeState = async (userId, currentState) => {
  try {
    ...
    if (isConfirmed) {
      ...
      const response = await fetch(
        `http://localhost:3009/ADB/ministerio/${userId}/state`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        Toast.fire({
          icon: "error",
          title: "Error al cambiar el estado del ministerio",
        });
        getAll();
      }
    }
...
const deleteUser = async (userId) => {
  try {
    ...
    if (isConfirmed) {
      ...
      const response = await fetch(
        `http://localhost:3009/ADB/ministerio_delete/${userId}`,
        {
          method: "DELETE",
          headers:{
            Authorization: `Bearer ${token}`,
