...
const Usersmodel = require('../model/tipo_egresos_modelo') // Importa el modelo ProductosModel
...
class Users {
    ...
    static async createUser(req, res) {
        try {
            ...
            if (result) {
                ...
                res.status(200).json({ message: 'Tipo de egreso creado correctamente' });
            } else {
                // Error al crear el proveedor en el modelo
                res.status(500).json({ error: 'Error al crear al tipo de egreso' });
            }
        } catch (error) {
            console.error('Error al crear al tipo de egreso:', error);
            res.status(500).json({ error: 'Error al crear al tipo de egreso' });
        }
    }
    ...
    static async updateUser(req, res) {
        try {
            ...
            const result = await Usersmodel.updateUser(id_tipo_egresos, nombre );

            // Verificar si el proveedor se actualizó correctamente en el modelo
            if (result) {
                // Proveedor actualizado correctamente
                res.status(200).json({ message: 'tipo de egreso actualizado correctamente' });
            } else {
                // Error al actualizar el provedor en el modelo
                res.status(500).json({ error: 'Error al actualizar el tipo de egreso' });
            }
        } catch (error) {
    ...
    static async changeState(req, res) {
        try {
            ...
            res.status(200).json({ message: 'Estado del tipo de egreso cambiado correctamente' });
        } catch (error) {
            console.error('Error al cambiar el estado del tipo de egreso:', error);
    ...
    static async deleteUser(req, res) {
        try {
            ...
            const responseObj = { message: "Tipo de Egreso eliminado correctamente" };
