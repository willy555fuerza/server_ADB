...
class Usersmodel {
  ...
  static async getAll() {
    try {
      ...
      if (result.rows.length === 0) {
        return { data: null, error: true, message: 'No hay tipo ingresos registrados' };
      }
  ...
  static async createUser(nombre) {
    ...
    try {
      ...
      console.log('Tipo de ingreso creado correctamente');
      return true;
    } catch (error) {
  ...
  static async updateUser(id_tipo_ingresos, nombre) {
    let pool;
    try {
      console.log(id_tipo_ingresos, nombre)
      // Conectar a la base de datos PostgreSQL
      pool = await connectToPostgres();
      if (!pool) {
        throw new Error('Error al conectar con PostgreSQL');
  ...
  static async changeState(userId, state) {
    try {
      ...
      await pool.query(`UPDATE tipo_ingresos SET estado = ${state} WHERE id_tipo_ingresos = ${userId}`);
      await disconnectFromPostgres(pool);
      return true;
    } catch (error) {
      return false;
